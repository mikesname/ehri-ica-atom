...
abstract class BaseActor extends QubitObject implements ArrayAccess
{
  ...
  public function offsetExists($offset, array $options = array())
  {
    ...
    if ($this->getCurrentactorI18n($options)->offsetExists($offset, $options))
    {
      return true;
    }

    if (!empty($options['cultureFallback']) && $this->getCurrentactorI18n(array('sourceCulture' => true) + $options)->offsetExists($offset, $options))
    {
      return true;
    }
  ...
  public function offsetGet($offset, array $options = array())
  {
    ...
    if (null !== $value = $this->getCurrentactorI18n($options)->offsetGet($offset, $options))
    {
      if (!empty($options['cultureFallback']) && 1 > strlen($value))
      {
        $value = $this->getCurrentactorI18n(array('sourceCulture' => true) + $options)->offsetGet($offset, $options);
      }

      return $value;
    }

    if (!empty($options['cultureFallback']) && null !== $value = $this->getCurrentactorI18n(array('sourceCulture' => true) + $options)->offsetGet($offset, $options))
    {
      return $value;
    }
  }
  ...
  public function offsetSet($offset, $value, array $options = array())
  {
    ...
    $this->getCurrentactorI18n($options)->offsetSet($offset, $value, $options);
  ...
  public function offsetUnset($offset, array $options = array())
  {
    ...
    $this->getCurrentactorI18n($options)->offsetUnset($offset, $options);
