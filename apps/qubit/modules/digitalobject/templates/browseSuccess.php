    ...
    <?php foreach ($pager->getResults() as $key => $item): ?>
      ...
      <td>
        ...
        <?php if ($item->showAsCompoundDigitalObject()): ?>
          <?php echo get_component('digitalobject', 'show', array('digitalObject' => $item->getPage(0), 'usageType' => QubitTerm::THUMBNAIL_ID, 'link' => array($item->informationObject, 'module' => 'informationobject'), 'iconOnly' => true)) ?>
        <?php else: ?>
          <?php echo get_component('digitalobject', 'show', array('digitalObject' => $item, 'usageType' => QubitTerm::THUMBNAIL_ID, 'link' => array($item->informationObject, 'module' => 'informationobject'), 'iconOnly' => true)) ?>
        <?php endif; ?>
