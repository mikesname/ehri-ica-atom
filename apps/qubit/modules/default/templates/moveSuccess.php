    ...
    <ol>
      ...
      <?php foreach ($parent->ancestors as $item): ?>
        <?php if (isset($item->parent)): ?>
          <li><?php echo link_to(render_title($item), array($resource, 'module' => 'default', 'action' => 'move', 'parent' => url_for(array($item, 'module' => 'informationobject')))) ?></li>
        <?php endif; ?>
      <?php endforeach; ?>
  ...
  <thead>
    ...
    <?php foreach ($results as $item): ?>
      <tr class="<?php echo 0 == @++$row % 2 ? 'even' : 'odd' ?>">
        <td>
          <?php echo link_to_if($resource->lft > $item->lft || $resource->rgt < $item->rgt, render_title($item), array($resource, 'module' => 'default', 'action' => 'move', 'parent' => url_for(array($item, 'module' => 'informationobject')))) ?>
        </td>
      </tr>
    <?php endforeach; ?>
  </tbody>
