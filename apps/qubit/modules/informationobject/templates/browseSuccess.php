...
<table class="sticky-enabled">
  ...
  <?php foreach ($pager->getResults() as $informationObject): ?>
    <tr class="<?php echo 0 == ++$row % 2 ? 'even' : 'odd' ?>">
      ...
      <?php if (QubitTerm::PUBLICATION_STATUS_DRAFT_ID == $informationObject->getPublicationStatus()->status->id): ?> 
        <span class="publicationStatus"><?php echo $informationObject->getPublicationStatus()->status ?></span>
      <?php endif; ?>
      </td>
      <td>
      <?php echo $informationObject->getLevelOfDescription() ?>
      </td>
      <td>    
      <?php if (('updatedUp' == $sf_request->sort) || ('updatedDown' == $sf_request->sort)): ?>
        <?php echo $informationObject->updatedAt ?>
      <?php elseif (sfConfig::get('app_multi_repository')): ?>
        <?php echo $informationObject->getRepository(array('inherit' => true)) ?>
      <?php else: ?>
        <ul>
        <?php foreach ($informationObject->getCreators(array('inherit' => true)) as $creator): ?>
...
<div class="search">
  <form action="<?php echo url_for(array('module' => 'search')) ?>">
    ...
    <?php echo submit_tag(__('Search ').sfConfig::get('app_ui_label_informationobject')) ?>
  </form>
</div>
